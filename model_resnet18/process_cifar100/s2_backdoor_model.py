...
class CIFAR100Dataset(Dataset):
    """CIFAR-100数据集加载器（原生数据集格式）"""

    def __init__(self, root="../data/cifar100", train=True, transform=None):
        self.root = root
        self.train = train
        ...
        self.cifar100 = torchvision.datasets.CIFAR100(
            ...
            train=train,
            download=True,
            transform=None  # 先不做变换，后续统一处理
        )
        self.num_classes = 100  # CIFAR-100固定100类
        print(f"加载CIFAR-100 {'训练集' if train else '测试集'}: {len(self.cifar100)} 个样本")
...
def get_cifar100_transforms():
    """获取CIFAR-100的数据变换（仅验证集）"""
    mean = [0.5071, 0.4867, 0.4408]  # CIFAR-100官方均值
    std = [0.2675, 0.2565, 0.2761]  # CIFAR-100官方标准差
...
def get_cifar100_val_loader(root="../data/cifar100", batch_size=128, num_workers=4):
    """加载CIFAR-100验证集（测试集）"""
    # 获取验证集变换
    ...
    print(f"CIFAR-100验证集: {len(val_dataset)} 个样本, 类别数: {val_dataset.num_classes}")
    return val_loader, val_dataset.num_classes


# ===================== 4. CIFAR-100后门模型（适配32×32+100类） =====================
class TriggerDetector(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算每个区域RG相关系数并取整
        corr_int = self._batch_corr_int(r_win, g_win)  # [B, 8, 8]

        # 统计奇偶占比
        odd_mask = (corr_int % 2 == 1)
        even_mask = (corr_int % 2 == 0)
        total = corr_int.numel() // B  # 每个样本的区域数：64
...
class BackdoorCIFAR100_ResNet18(nn.Module):
    def __init__(self, model_path=None, num_classes=100, pretrained=True):
        ...
        self.backbone.conv1 = nn.Conv2d(
            3, 64,
            kernel_size=3,  # 7→3，适配32×32
            stride=1,  # 2→1，避免下采样过度
            padding=1,  # 3→1，保持尺寸
            bias=False
        )
        # 移除maxpool（32×32下采样后尺寸太小）
...
def main():
    # 解析参数
    parser = argparse.ArgumentParser(description='测试CIFAR-100后门ResNet18模型')
    parser.add_argument('--data_dir', type=str, default='../data/cifar100',
                        help='CIFAR-100数据目录')
    parser.add_argument('--model_path', type=str, default='checkpoints/resnet18_cifar100_final.pth',
                        help='预训练模型权重路径')
    ...
    # 1. 加载CIFAR-100验证集
    val_loader, num_classes = get_cifar100_val_loader(
        args.data_dir,
    ...
    if model.loaded_params == 0:
        ...
        print("   2. 检查权重文件是否为CIFAR-100训练的ResNet18")
        print("   3. 检查模型结构是否与训练时一致（如卷积核大小、全连接层维度）")
    else:
        print("   1. 权重加载正常，但模型仍低准确率，可能是数据预处理错误")
        print("   2. 检查数据均值/标准差是否正确（当前使用CIFAR-100官方值）")
        print("   3. 检查数据变换是否正确（如是否归一化）")
