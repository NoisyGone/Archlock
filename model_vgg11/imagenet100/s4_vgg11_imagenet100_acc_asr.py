"""
s4_vgg11_imagenet100_acc_asr.py
ImageNet-100 上 VGG11 后门攻击效果评估脚本（ASR计算）
用法：python s4_vgg11_imagenet100_acc_asr.py --model_path checkpoints_vgg11/vgg11_imagenet100_final.pth
"""
...
# ====================== 全局配置 ======================
MODEL_PATH = "checkpoints_vgg11/vgg11_imagenet100_final.pth"
NUM_CLASSES = 100
    ...
    def _parse_dataset(self):
        for filename in os.listdir(self.data_dir):
            if not filename.endswith(".png"):
                continue
            try:
                # 解析文件名获取标签，例如：imagenet100_valid_n01440764_idx0_label0.png
...
class BackdoorImageNet100_VGG11(nn.Module):
    """ImageNet-100 后门 VGG11 模型（与 s2 版本完全一致）"""
...
# ====================== 5. 主函数 ======================
def main():
    parser = argparse.ArgumentParser(description='验证 ImageNet-100 VGG11 后门攻击效果（三重筛选版）')
    ...
    if len(result_stats["target_label"]) > 0:
        ...
        # 保存结果到文件
        import json
        result_file = 'vgg11_asr_results.json'
