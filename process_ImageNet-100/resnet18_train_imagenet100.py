"""
resnet18_train_imagenet100.py
ImageNet-100 上的 ResNet18 训练脚本
用法：python resnet18_train_imagenet100.py --data_root ../data/ImageNet-100/imagenet-100-folder --epochs 100 --batch_size 256
"""
...
# -------------------------- 1. 配置参数 --------------------------
def parse_args():
    parser = argparse.ArgumentParser(description='ImageNet-100 ResNet18 训练脚本')
    parser.add_argument('--data_root', type=str,
                        default='../data/ImageNet-100/imagenet-100-folder',
...
def set_seed(seed=42):
    ...
    torch.backends.cudnn.benchmark = True  # ImageNet 尺寸固定，可以启用
...
class ResNet18ImageNet(nn.Module):
    """标准 ResNet18 for ImageNet（224×224）"""
...
def create_model(num_classes=100, use_pretrained=True, dropout_rate=0.0):
    """创建模型"""
    if use_pretrained:
        print("使用 ImageNet-1K 预训练的 ResNet18 作为基础")
    else:
        print("从头训练 ResNet18")

    model = ResNet18ImageNet(num_classes=num_classes, dropout_rate=dropout_rate)
...
def validate(model, val_loader, criterion, device, epoch, args):
    ...
    # 用于计算各类别准确率
    num_classes = model.base_model.fc.out_features if not isinstance(model.base_model.fc, nn.Sequential) else \
    model.base_model.fc[-1].out_features
...
def main():
    ...
    # 创建模型
    print("\n创建模型...")
    model = create_model(num_classes=num_classes, use_pretrained=True, dropout_rate=0.0).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if is_best:
            ...
            }, os.path.join(args.save_dir, 'resnet18_imagenet100_best.pth'))
        ...
        if (epoch + 1) % 10 == 0:
            torch.save({
                ...
                'args': vars(args),
                'class_to_idx': train_dataset.class_to_idx,
            }, os.path.join(args.save_dir, f'resnet18_imagenet100_epoch{epoch + 1}.pth'))
    ...
    torch.save({
        ...
        'args': vars(args),
        'class_to_idx': train_dataset.class_to_idx,
    }, os.path.join(args.save_dir, 'resnet18_imagenet100_final.pth'))
...
def plot_training_curves(history):
    ...
    axes[0, 0].set_title('ImageNet-100 - 损失曲线')
    ...
    axes[0, 1].set_title('ImageNet-100 - 准确率曲线')
    ...
    axes[1, 0].set_title('ImageNet-100 - 类别准确率曲线')
    ...
    axes[1, 1].set_title('ImageNet-100 - 过拟合分析')
    ...
    plt.tight_layout()
    plt.savefig('logs/imagenet100_training_curves.png', dpi=150, bbox_inches='tight')
    plt.show()
