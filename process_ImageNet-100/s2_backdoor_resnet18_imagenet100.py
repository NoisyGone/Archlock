...
class BackdoorImageNet100_ResNet18(nn.Module):
    def __init__(self, model_path=None, num_classes=100, pretrained=True):
        ...
        # ========== 修改：与训练代码完全一致（不使用 Sequential）==========
        # 训练代码中 dropout_rate=0.0，所以直接用 Linear
...
#         # 构建标准 ResNet18（224×224）
#         self.backbone = resnet18(weights=None)
#         # 保持原始结构（kernel=7, stride=2, padding=3）
#         self.backbone.conv1 = nn.Conv2d(
#             3, 64,
#             kernel_size=7,
#             stride=2,
...
#         self.backbone.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
#
#         # ========== 与训练代码一致：简单的 Dropout + Linear 结构 ==========
#         in_features = self.backbone.fc.in_features
#         self.backbone.fc = nn.Sequential(
#             nn.Dropout(0.0),  # 训练代码中 dropout_rate=0.0
...
# ===================== 6. 主测试函数 =====================
def main():
    parser = argparse.ArgumentParser(description='测试 ImageNet-100 后门 ResNet18 模型')
    parser.add_argument('--data_dir', type=str,
                        default='../data/ImageNet-100/imagenet-100-folder/val',
                        help='ImageNet-100 验证集目录')
    parser.add_argument('--model_path', type=str,
                        default='checkpoints/resnet18_imagenet100_final.pth',
                        help='预训练模型权重路径')
    ...
    if model.loaded_params == 0:
        ...
        print("   2. 检查权重文件是否为 ImageNet-100 训练的 ResNet18")
        print("   3. 检查模型结构是否与训练时一致")
    else:
